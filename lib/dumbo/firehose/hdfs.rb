    ...
    class HDFS
      ...
      def slots!(topic, namespace, interval)
        ...
        enumerable_interval(interval).map do |time|
          SlotOptions.new(@sources, @hdfs, topic, namespace, Time.at(time).utc).best_slot
        end.reject do |slot|
          slot.events.to_i < 1
        end
      end

      def slots_options!(topic, namespace, interval)
        $log.info("scanning HDFS options for", interval: interval)
        enumerable_interval(interval).map do |time|
          SlotOptions.new(@sources, @hdfs, topic, namespace, Time.at(time).utc)
        end.reject do |slot|
      ...
      def namespaces(hdfs_root)
        folders = @hdfs.first.list(hdfs_root).select do |entry|
          entry['pathSuffix'] =~ /\A[0-9]*\Z/ || entry['pathSuffix'] == 'default'
        end
      ...
      class SlotOptions
        ...
        def initialize(sources, hdfs_servers, topic, namespace, time)
          @sources = sources
        ...
        def all!
          @hdfs_servers.map do |hdfs|
            Slot.new(@sources, hdfs, @topic, @namespace, @time)
          end
        end
        ...
        def synchronize!(dryrun = true)
          $log.info("synchronizing slot options at", time: @time, topic: @topic, namespace: @namespace)
